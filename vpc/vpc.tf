...
resource "aws_vpc" "vpc_name" {
  ...
  tags = "${merge(map("Name", format("%s", var.vpc_name)), var.custom_tags)}"
}
...
resource "aws_subnet" "vpc_public_sn" {
  ...
  vpc_id = "${aws_vpc.vpc_name.id}"
  cidr_block = "${lookup(var.vpc_public_subnet_cidr, "AZ-${count.index}", cidrsubnet(var.vpc_cidr_block, 5, count.index))}"
  availability_zone = "${element(data.aws_availability_zones.available.names, count.index)}"
  tags = "${merge(map("Name", "${var.vpc_name}-public-${count.index}"), var.custom_tags)}"
}

# Routing table for public subnet
resource "aws_route_table" "public_rt" {
  vpc_id = "${aws_vpc.vpc_name.id}"
  tags   = "${merge(map("Name", "${var.vpc_name}-public"), var.custom_tags)}"
}
...
resource "aws_route" "internet" {
  ...
  depends_on = ["aws_internet_gateway.public_ig", "aws_route_table.public_rt"]
...
resource "aws_subnet" "vpc_private_sn" {
  count = "${data.template_file.num_availability_zones.rendered}"
  vpc_id = "${aws_vpc.vpc_name.id}"
  cidr_block = "${lookup(var.vpc_private_subnet_cidr, "AZ-${count.index}", cidrsubnet(var.vpc_cidr_block, 5, count.index))}"
  availability_zone = "${element(data.aws_availability_zones.available.names, count.index)}"
  tags = "${merge(map("Name", "${var.vpc_name}-private-${count.index}"), var.custom_tags)}"
}
...
resource "aws_route" "nat" {
  ...
  depends_on = ["aws_internet_gateway.public_ig", "aws_route_table.private_rt"]
